# ArbScanner PoC 0.9.57

0.9.57 is a closure-only Dashboard upgrade/mode-integrity repair on top of 0.9.56. It restores the additive routing/settlement audit migration for mature databases so the Dashboard core financial route cannot fail on missing 0.9.56 schema objects, makes routing diagnostics fail-soft, and prevents stale LIVE Dashboard clears from blanking SIM after a mode switch. 0.9.56 routing/settlement economics and Scenarios layout remain unchanged; LIVE order writing remains centrally locked.
